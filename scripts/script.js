$(document).ready(function() {
  ...
  $(".order-delete").click(function() {
    let orderId = $(this)
      ...
      .find("td:first-child")
      .text();
    var cnfrmCancellation = confirm(
      "Do you really want to cancel order ?\n Click Ok to cancel Order."
    );
    if (cnfrmCancellation == true) {
      var posting = $.post("/handlers/formhandle/order/update-order/", {
        orderId: orderId
      });
...
window.onload = function() {
  ...
  if (orderForm) {
    ...
    for (var Profession in professionInfo) {
      professionServiceSel.options[
        professionServiceSel.options.length
      ] = new Option(Profession, Profession);
    }
    ...
    professionServiceSel.onchange = function setProfessionType() {
      ...
      for (var professionName in professionInfo[this.value]) {
        professionNameSel.options[
          professionNameSel.options.length
        ] = new Option(professionName, professionName);
      }
    };
    ...
    professionNameSel.onchange = function() {
      ...
      for (var professionType in professionInfo[professionServiceSel.value][
        this.value
      ]) {
        professionTypeSel.options[
          professionTypeSel.options.length
        ] = new Option(professionType, professionType);
      }
    };
    //WorkType Changed
    professionTypeSel.onchange = function() {
      ...
      var roles =
        professionInfo[professionServiceSel.value][professionNameSel.value][
          this.value
        ];
      for (var i = 0; i < roles.length; i++) {
        professionRoleSel.options[
          professionRoleSel.options.length
        ] = new Option(roles[i], roles[i]);
      }
    };
    ...
    searchInp.onkeyup = function() {
      ...
      if (searchStr) {
        ...
        for (var i = 0; i < searchResLen; i++) {
          ...
          nodeEle.href =
            "/service-providers/" +
            arrTextVal.replace(/\s+/g, "-").toLowerCase();
